...
create_rates <- function(filename, pop, epop_total, ind_id) {
  ...
  data_indicator %<>%
    mutate(epop = recode(as.character(age_grp),
           ...
           epop = case_when(sex_grp == "All" ~ epop*2, T ~ epop)) ### [I think this is incorrect] ###
...
###############################################.
# Function to calculate age sex standardized rates by sex
# Parameters:
# filename : name of indicator being produced
# pop: specify which population (e.g. allages, 65+, 0to64 to ensure populations match)
# epop_total: figure to use for total european standard pop (use the figure for one gender only)
...
create_rates_by_sex <- function(filename, pop, epop_total, ind_id) {
...
###############################################.
# Function to calculate age sex standardized rates females only (ie for cervical cancer)
# Parameters:
# filename : name of indicator being produced
# pop: specify which population (e.g. allages, 65+, 0to64 to ensure populations match)
# epop_total: figure to use for total european standard pop (use the figure for one gender only)
...
create_rates_female <- function(filename, pop, epop_total, ind_id) {
...
create_pop <- function(lower, upper, name) {
  ...
  scot_pop_sex <- readRDS('/conf/linkage/output/lookups/Unicode/Populations/Estimates/HB2019_pop_est_1981_2021.rds') %>%
    setNames(tolower(names(.))) %>% # variables to lower case
